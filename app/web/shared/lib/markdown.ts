...
/**
 * 把正文里的相对资源路径重写为绝对路径。
 * `map` 返回 null 表示这条不改写。用于把 `bbs/x.png` 指到 `/forum/r/bbs/x.png`。
 */
function rewriteResources(html: string, map: (path: string) => string | null): string {
  return html.replace(/(src|href)=("|')([^"':][^"']*)\2/g, (whole, attr, quote, path) => {
    const mapped = map(path);
    return mapped ? `${attr}=${quote}${mapped}${quote}` : whole;
  });
}
...
/**
 * 渲染 Markdown 为净化后的 HTML。全站唯一的 Markdown 入口 ——
 * 论坛帖正文与官网文档站都走这里，避免出现第二份未净化的实现。
 */
export function renderMarkdown(content: string, options: MarkdownOptions = {}): string {
  if (!content) return "";
  const html = marked.parse(content) as string;
  const rewritten = options.resolveAsset ? rewriteResources(html, options.resolveAsset) : html;
...
/** 论坛帖正文：把 mbbs 迁移过来的 `bbs/` 附件指到 nginx 的 `/forum/r/` 静态目录。 */
export function renderForumContent(content: string, _format?: "markdown" | "html" | string): string {
  return renderMarkdown(content, {
    resolveAsset: (p) => (p.startsWith("bbs/") ? `/forum/r/${p}` : null),
  });
}
