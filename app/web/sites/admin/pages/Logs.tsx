...
export default function Logs() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["logs", org],
    queryFn: () => api<{ logs: any[] }>(`/api/admin/${org}/logs`),
  });
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-ink-800/60">
            {data!.logs.length === 0 && <tr><td colSpan={6} className="px-5 py-8 text-center text-ink-500">无日志</td></tr>}
            {data!.logs.map((l: any) => (
              <tr key={l.id} className="hover:bg-ink-800/30">
                ...
                <td className="px-5 py-3 font-mono text-ink-300 max-w-xs truncate">{l.target ?? "—"}</td>
