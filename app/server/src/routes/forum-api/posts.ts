...
import { FORUM_LIMITS, ForumError, GUEST_POST_SITE_SUBJECT, MIXED_SCRIPT_MESSAGE, ipSubject, nameProblem } from "../../lib/forum-rules.js";
...
export default async function forumPostRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: ReplyBody }>("/api/forum/posts", async (req, reply) => {
    ...
    const problem = name ? nameProblem(name) : "hidden";
    if (problem === "mixed_script") throw new ForumError(400, "invalid_guest_name", MIXED_SCRIPT_MESSAGE);
    if (problem || name.length > FORUM_LIMITS.guestNameMax) {
