import { Link, useNavigate, useSearchParams } from "react-router-dom";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { useState } from "react";
import { api } from "../../lib/api";

export default function ForumRegister() {
  const nav = useNavigate();
  const qc = useQueryClient();
  const [params] = useSearchParams();
  const returnTo = params.get("return_to") ?? "/forum";
  const [form, setForm] = useState({ username: "", display_name: "", password: "", email: "" });

  const submit = useMutation({
    mutationFn: () => api(`/api/forum/auth/register`, { method: "POST", body: JSON.stringify(form) }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["forum-me"] }); nav(returnTo); },
  });

  return (
    <div className="max-w-md mx-auto px-6 py-10">
      <h1 className="text-2xl font-bold text-ink-50 mb-1">注册</h1>
      <p className="text-ink-300 text-sm mb-6">
        已有账号？<Link to={`/forum/login?return_to=${encodeURIComponent(returnTo)}`} className="text-brand-500 hover:underline">去登录</Link>
      </p>

      <div className="card p-6">
        <a href={`/auth/forum/github?return_to=${encodeURIComponent(returnTo)}`}
          className="w-full inline-flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg bg-ink-50 text-white hover:opacity-90 transition text-sm font-medium">
          <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path d="M12 .5C5.65.5.5 5.65.5 12c0 5.08 3.29 9.39 7.86 10.91.57.11.78-.25.78-.55v-1.95c-3.2.7-3.87-1.36-3.87-1.36-.52-1.33-1.28-1.68-1.28-1.68-1.05-.72.08-.7.08-.7 1.16.08 1.77 1.19 1.77 1.19 1.03 1.77 2.71 1.26 3.37.96.1-.75.4-1.26.73-1.55-2.55-.29-5.24-1.27-5.24-5.66 0-1.25.45-2.27 1.18-3.07-.12-.29-.51-1.46.11-3.04 0 0 .97-.31 3.18 1.18a11 11 0 015.79 0c2.21-1.49 3.18-1.18 3.18-1.18.62 1.58.23 2.75.11 3.04.74.8 1.18 1.82 1.18 3.07 0 4.4-2.69 5.36-5.25 5.65.41.36.78 1.06.78 2.13v3.16c0 .31.21.67.79.55C20.21 21.38 23.5 17.08 23.5 12 23.5 5.65 18.35.5 12 .5z"/></svg>
          GitHub 一键注册并登录
        </a>
        <div className="flex items-center gap-3 my-5 text-xs text-ink-400">
          <div className="flex-1 h-px bg-brand-500/15" />
          <span>或用账号密码注册</span>
          <div className="flex-1 h-px bg-brand-500/15" />
        </div>
        <form onSubmit={(e) => { e.preventDefault(); submit.mutate(); }} className="space-y-3">
          <div>
            <label className="label text-sm">用户名 · 2-32 字符</label>
            <input autoFocus className="input" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} required minLength={2} maxLength={32} />
          </div>
          <div>
            <label className="label text-sm">昵称 (可选)</label>
            <input className="input" value={form.display_name} onChange={(e) => setForm({ ...form, display_name: e.target.value })} maxLength={64} />
          </div>
          <div>
            <label className="label text-sm">邮箱 (可选)</label>
            <input type="email" className="input" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
          </div>
          <div>
            <label className="label text-sm">密码 · 至少 6 位</label>
            <input type="password" className="input" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required minLength={6} />
          </div>
          {submit.error && <div className="text-rose-500 text-sm">{(submit.error as Error).message}</div>}
          <button type="submit" disabled={submit.isPending || !form.username || !form.password}
            className="btn-primary w-full py-2.5 mt-2 disabled:opacity-50">
            {submit.isPending ? "注册中…" : "创建账号"}
          </button>
        </form>
      </div>
    </div>
  );
}
