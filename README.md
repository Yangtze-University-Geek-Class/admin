...
> [!IMPORTANT]
> **预发布与正式是两套一模一样的完整服务，只是互相隔离。** 每套都有 web、server（Fastify + 自己的 SQLite）、forum 三个容器，各用各的域名、密钥和 GitHub OAuth 应用。官网、论坛、控制台共用同一个 GitHub 登录（一个 `sid` cookie，只有极客班 GitHub 组织的成员能登录成功）。部署的镜像都是生产构建，不含样板数据；样板数据只在本机开发态使用。**论坛还不能发帖和回复**：帖子还没有服务端存储，线上只有极客班自己的分类、标签和放出来的旧帖。登录后的 3D Hub 也还没做。
...
| 没有 | 论坛发帖、回复的服务端存储；登录后的 3D Hub；正式环境的自动部署（正式发布由维护者在所有者验收后部署） |
