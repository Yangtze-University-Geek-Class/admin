...
14. **执行记录**：`node scripts/note.mjs check --pr --base origin/stage --head <task 分支>` 必须通过；再打开 `notes/<日期>/<用户名>/<链路>.md` 对照 `git log`、PR 和 CI 运行号，核对记录的 SHA、命令输出、时间与实际一致，没有补写没发生过的事（`docs/conventions/NOTES.md`）。
