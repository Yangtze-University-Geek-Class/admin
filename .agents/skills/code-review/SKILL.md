...
15. **合并后清理**（合并之后由合并的人当场跑，所有者 2026-09-26 定的强制规范）：`gh issue view <issue> --json state` 必须是 `CLOSED`；`git ls-remote --heads origin 'task/<issue>/*'` 必须没有输出；只读运行 `node scripts/issue-sweep.mjs`，报告里不能有「将补关」（合并不到一小时的 PR 它先不管，刚合并时以 `gh issue view` 为准）；开发者本机 `node scripts/task.mjs list --check` 必须通过。没关上的（例如 fork PR 合并时工作流没有写权限，#137）当场手工关并按 `docs/conventions/TRACKING.md` §3 留「关闭」记录（`docs/conventions/CODE-REVIEW.md` 第 12 项）。
