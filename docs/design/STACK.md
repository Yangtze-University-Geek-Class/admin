...
| 层 | 当前方案 | 版本线 |
|---|---|---|
| 工作区 | pnpm workspace，`app/web`、`app/server` 两包（论坛独立） | pnpm 9.15.9 |
...
既有 `app/web` 包仍是 React/Node 22 环境；其他新增或并行迁移的服务应分别核对实际 manifest。Vue 组件不能直接当作 React 组件使用。组件文档建设与论坛代码接入是不同变更，实际迁移状态以各服务代码和验收为准。
