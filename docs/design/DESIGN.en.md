...
TuffEx is the selected UI foundation. The forum now directly uses its Nuxt/Vue upstream source; existing portal/admin React adapters remain transitional. Mock forum interactions are browser-local and do not prove real authentication or production readiness. Production rollout, external authentication and operational verification require separate authorization and evidence.
