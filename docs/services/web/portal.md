...
- **点下去**：`components/os/Wallpaper.tsx` 马上压上新的一层：底色加放大、模糊的缩略图（选择面板里已经加载好，不用等网络）。这一层从点的那张缩略图的位置展开到整个桌面：起点是缩略图在壁纸层里的矩形（`revealClipFrom` 算成 `clip-path: inset(… round 9px)`，和缩略图圆角一样），终点是整个桌面，640ms，缓出曲线 `cubic-bezier(.22, 1, .36, 1)`。大图下载、解码好（`loadWallpaperImage`，`image.decode()`）以后在这一层里淡入 480ms，模糊的画面清晰过来，再卸掉下面的缩略图。已经解码过的大图（开机那张、换过的、预取过的）直接给大图，不先顶缩略图。
...
- **单测**：`tests/web/portal-wallpapers.test.ts`（清单、展开起点、预取顺序与条件），`tests/web/portal-wallpaper-switch.test.tsx`（jsdom：占位立刻出现、解码后换上大图、连点停在最后一张且不提前卸层、减少动态效果走淡入、下载失败停在占位、开机那张只下一次；预取的顺序、取消、去重与省流量不预取）。
