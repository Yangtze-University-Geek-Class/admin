...
| `components/os/*` | YUGC OS 桌面：应用图标与「新来的看这里」便签（`Widgets.tsx`）、窗口、菜单栏、Dock、启动器；壁纸图层与换壁纸面板在 `Wallpaper.tsx`，壁纸清单在 `lib/wallpapers.ts`，文件在 `public/portal/wallpapers/`（每张一张静态图 + 一段循环视频，换视频后跑 `node scripts/wallpaper-qa.mjs` 逐帧验收） |
