...
| `app/web` 镜像内 nginx 配置 | 由 `Dockerfile` 生成：`/api/*`、`/auth`、`/auth/*`（OAuth 登录与回调）、`/healthz` → `server:3000`；`/forum` 308 到 `/forum/`，`/forum/*` 剥掉前缀后 → `forum:3000`；`/admin`、`/console` 及其子路径与 `/signin` → 控制台入口 `sites/console/index.html`，其余 → portal，与 Host 无关；`/assets/*`（官网）与 `/console-assets/*`（控制台）是带哈希的长缓存资源；`/sites/*` 只提供真实文件，不存在即 404；安全头由宿主 nginx 统一下发，容器不重复 |
