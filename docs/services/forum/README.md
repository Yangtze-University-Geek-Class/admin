...
- 断言：`tests/site-state.test.ts` 核对 17 篇的类别、标签、置顶、作者和编号小于 1000（论坛后端按编号导入旧帖，新话题从 `t1001` 起，#57），站内链接只指向公开话题、图片文件都在，正文里没有邮箱、写出来的账户和 API 密钥；镜像构建断言 `t/t73.md`、`t/t9.md`、`t/t78.md`、`t/t84.md`、`t/t89.md` 存在，`t/t5.md`、`t/t15.md` 不存在，`published/` 里至少有一张 webp 图片，产物的文本文件里没有邮箱地址和 `sk-` 开头的密钥（`grep -I` 跳过图片和字体）。
...
- **写**：每个写操作调一个接口，服务端只回这次改动的记录（`{ changes, viewer, guestPolicy }`，#145，形状见 [API](../../architecture/API.md)「论坛」「写接口的回答」）。`shared/forum-api.ts` 的 `parseWriteResult` 先查形状，哪里不对整份不收、当作失败；`stores/forum.ts` 的 `applyChanges` 按编号并进手里的状态：已有的记录逐字段改、对象不换，新记录追加，取消的书签和关注按 `removed` 去掉，所以点一次赞只重画这一个按钮，不再换掉整个 store、整页重画。`viewer` 和 `guestPolicy` 只在变了时才换；回答里的 `viewer` 换了人（登录在别处变了）时再读一次状态。回答里只有这次写入动到的记录，不再顺带带回别人的新动态：别人刚发的回复、别人给你的通知和未读数，要等页面重新读状态才出现，而重读只在打开页面（或刷新）、退出、回 401、429 之后重试、`viewer` 换人时发生，没有定时重读，也不在切回标签页时重读。所以两个人在同一个话题里，B 先回复、A 再回复，A 的页面上看不到 B 的回复，A 自己的回复按页面上已有的帖子排楼层（刷新后可能往后挪一层；「回复 @x」的链接不带楼层号，不受影响），B 给 A 的通知也要刷新才有。这不丢数据，也不会让谁看到不该看的；要不要加一次轻量重读（切回标签页时，或话题页按话题取帖子）放到拆首屏的 #156 里定，不恢复写接口回整份状态。
- **先显示再发请求**（#145）：点赞、书签、关注、编辑、删除、置顶、关闭、标为已读、全部已读和账号资料，点下去页面马上变，请求在后面发（`stores/forum-server.ts` 的 `steer`）。同一件事（同一条帖子的赞、同一个话题的置顶、自己的资料……）同时只有一个请求在路上：路上时再点只改页面，回答到了，最后一次点的和服务端现在的不一样才再发一次，所以连点不会同时发出两个请求，也不会停在和服务端不一样的样子；别的事的回答里带着这条记录的旧值时，还在路上的这件事照样显示点过之后的样子。资料只发和服务端上次确认的不一样的字段。回复也先显示：帖子（游客还有昵称）以 `pending:` 开头的编号先放进话题，回答到了在同一拍换成服务端的帖子；还在发的回复卡片上写「发送中」（卡片带 `aria-busy="true"`），不显示赞、链接、书签、编辑、删除和回复，换成服务端的帖子以后才有；写入口对 `pending:` 编号的帖子也不发请求，只弹「这条回复还在发送」（`tests/forum-optimistic-components.test.ts`、`tests/forum-optimistic.test.ts`）。新话题要等服务端的编号才能跳过去，头像要等服务端存好的图片，这两件仍然等回答。
- **失败**：页面回到服务端上次确认的样子，弹 Tuffex toast（标题说哪件事没成，比如「没有赞上」「没有移出书签」「回复没有发出去」，正文是服务端的中文 `message`，没有时按状态码给一句）；编辑失败时编辑框带着刚才写的内容重新打开；等回答时又改了一次再保存，两次共用一个请求、拿到同一个结果，`PostCard` 给保存编号，只让最后一次处理：「帖子已更新」只弹一次，被拒时编辑框带着最后一次的文字打开。回复失败时，回复框带着那条回复重新打开，回复的还是原来那一楼（`ReplyComposer` 发 `update:replyTo`，话题页用 `v-model:reply-to`）：框里这时已经有字（等回答时又点了别的楼的「回复」、写了下一条）就把被拒的放在前面、空一行；连着被拒几条，几条都在；几条写给不同的楼、或者框里的草稿写给另一楼时，文字都在，回复对象换成最后被拒那条的那一楼，标题随之改变；发出以后离开了话题页，下次打开这个话题时回复框自己带着它打开。被拒的回复先交给 `stores/forum-server.ts` 的 `refusedReplies`，由这个话题的回复框取回，所以不靠组件还在不在（`tests/forum-optimistic-components.test.ts`）。退回的只是这件事自己改的字段，值取服务端最后一次回答里的（`steer` 的 `confirmed`：帖子的正文与删除、通知的已读记在 `serverPosts`、`serverReads` 里，不从页面读），同一条记录上别的还在路上的事随后照样显示：先改后删同一帖、两个都失败时回到服务端的原文且没删；先标一条已读再点全部已读、两个都失败时两条都回到未读；连点两次赞、第二次失败时停在第一次服务端确认的样子，而不是点之前的样子（`tests/forum-optimistic.test.ts`）。页面上的变化点下就有；成功提示（「已加入书签」「话题已置顶」「话题已关闭」「帖子已删除」「回复已发布」「帖子已更新」「资料已保存」等）都等服务端确认后才弹，被拒时只有失败提示（#162）。书签、置顶、关闭、删除的提示按记录带 id（如 `forum-bookmark:<postId>`），快速连点几次等的是同一个结果，也只留一条提示，说的是服务端最后确认的状态。回复被拒时，说明原因的提示先弹、回复框随后打开：Tuffex 的提示和抽屉共用一个 z-index 计数器，后打开的抽屉连同遮罩会盖住提示，所以回复框打开后再给提示取一次 z-index（`toastStore.zIndex = nextZIndex()`），提示回到最上面；回复框开着时提示显示在顶部居中（`useShell().composerOpen`，`app.vue` 的 `TxToastHost`），不压住回复框右下角的「取消」「回复」。回 401（登录已经失效或在别处退出）时再读一次状态，页面换成游客能做的事（顶栏的登录状态要刷新页面才跟上）。429 与其它错误一样回到确认的样子并提示，服务端的限流（#126）不变。浏览数用 `POST /api/forum/topics/:id/view`（同一浏览器会话每个话题只报一次，服务端再按 IP 一小时去重），失败不提示。退出后重新读一次状态，书签和通知随之清空。
- **游客**（没登录）：能看全部帖子；在没关闭的话题里能回复：回复框多一个「昵称」（≤`guestPolicy.nameMax`，20，旁边写着能用的字符，与资料页同一句 `NAME_CHARS_HINT`），正文 ≤`guestPolicy.contentMax`（2000），按钮写「以游客身份回复」；发送前在浏览器里用 Web Crypto 算工作量证明（`shared/pow.ts`：找 nonce 使 `sha256("<timestamp>:<topicId>:<正文>:<nonce>")` 以 `powDifficulty` 个 0 开头，与核心服务 `middleware/pow.ts` 同一算法），蜜罐字段 `website` 恒为空串；服务端配了 Turnstile（`guestPolicy.turnstileSiteKey` 非空）时回复框下面多一个人机验证（`TurnstileBox.vue`，脚本来自 `challenges.cloudflare.com`，站点 CSP 已放行），令牌一次性，每次发送后换新。昵称和成员撞了、昵称里有服务端不收的字符、太频繁、全站游客回复暂停（429 `guest_replies_paused`）、证明失效都由服务端说明原因，toast 正文就是服务端的 `message`。游客看不到「新话题」；`/new` 显示「登录后才能发帖」和「用 GitHub 登录」；「赞」「书签」「关注」弹 toast「登录后才能继续」并带「登录」按钮；书签、通知、资料页显示「登录后才能……」和登录按钮。游客回复在帖子上署名后面跟一个「游客」（普通的次要文字，不是徽章），`/users` 不列游客，游客的个人页没有「关注」，也没有可改的资料。
...
- **点赞**（#143）：每条帖子（包括话题第一帖）操作栏最左边是「赞」按钮，Tuffex `TxButton` 的 `flat` 样式（常规文字色加边框，和旁边的「链接」一样清楚，不用更浅的 `secondary`）：心形图标加「赞」和赞数，例如「赞 3」，没人赞时只写「赞」。按钮不另设 `aria-label`，看得见的文字就是它的名字，读屏也读到赞数。自己赞过的，心形变实心、文字和边框变成红色（`type="danger"` 色调），并带 `aria-pressed`；游客看到的赞数一样，按钮不会是按下的样子。按钮显示什么、点了做什么都在 `data/likes.ts` 的 `likeControl`。成员点一下，按钮当场变（上面「先显示再发请求」），请求 `POST /api/forum/posts/:id/like` 在后面发，回答里只有这条帖子和自己的用户记录，按编号并进 store；服务端把赞存在 `forum_likes` 表里，刷新或换人读都还在。按钮只管显示和把点击交给 `useForumActions().toggleLike`，写入怎么走（多快变、连点两下怎么办）在写入口里定。游客点「赞」弹「登录后才能继续」和「登录」按钮；服务连不上、请求太频繁、账号不在组织里时说明原因，提示的文字在 `data/access.ts` 的 `loginPromptToast`。话题列表不显示赞数：列表照 Discourse 只有回复、浏览、活动三列，上游验收逐字核对这几列，加一列会挤窄标题，只在手机上加又会让两种列表对不上。
...
- **账号资料**（`/u/<用户名>/preferences`，标题「账号资料」）：昵称（≤30）、个人签名（≤200，可以换行，多行输入框）、所在地（≤60）、个人网站（空或 `https://`，≤200）与三项通知开关，保存前按这些上限先查一遍再 `PATCH /api/forum/me/profile`；昵称只在改了时查、时发（`shared/forum-api.ts` 的 `profileProblem` 与 `profileBody`）：新成员的默认昵称是 GitHub 登录名，最长 39 个字，超过改昵称的上限 30，不改昵称也要能保存签名这些，服务端对发来的昵称还要重新校验和查重。昵称下面写着能用的字符（`NAME_CHARS_HINT`，与核心服务 `NAME_RULE_MESSAGE` 同一条规则），浏览器不查字符，服务端拒绝时（400 `invalid_display_name`、`display_name_taken`）toast 正文是服务端的原话；头像页选一张 PNG、JPEG 或 WebP（≤2MB，浏览器先查类型和大小），用 `data:` 地址预览（站点 CSP 不放行 `blob:` 图片），「上传头像」把文件本身 `PUT /api/forum/me/avatar`，服务端裁成 256×256；「不用这张」放弃这次选择；上传过的头像可以「恢复 GitHub 头像」（`DELETE /api/forum/me/avatar`）。个人签名显示在个人主页横幅里，保留换行；个人网站只有 `https://` 开头时才显示成链接（`linkableWebsite`，快照或旧数据里的其它地址不显示）。头像在帖子、列表、菜单里都用 `avatarUrl`（上传的或 GitHub 的），没有时是首字母头像。
- **正文安全**：帖子正文（游客也能写）经 `ForumMarkdown.vue` 渲染：先过 `shared/post-markdown.ts`，原始 HTML 一律当文字显示（`<` 后面插一个不可见的 U+2060，复制时去掉），Typora 式 `<img>` 换成 Markdown 图片（只留 src 与 alt），链接和图片地址只放行 http(s)、mailto 与站内地址，`javascript:`、`data:`、`vbscript:` 等（含实体、控制字符写法）前面加 `#` 变成页内锚点；再交给 `TxMarkdownView`，它默认用 DOMPurify 再清一遍。单测 `tests/post-markdown.test.ts`。
- **写正文**：新话题（`/new`）、回复框（含引用）和编辑帖子都用 `PostEditor.vue`（#144），上方三个模式，默认「编辑」：「编辑」是纯文本框，直接写 Markdown；「分栏」左边文本框、右边渲染结果，随输入更新，编辑器宽度不到 42rem（手机、窄屏、话题页里的编辑框）时改成上下排，两块平分原来的高度，所以回复抽屉里也放得下（会员回复抽屉高 460px，原来 420px 放不下模式切换加 240px 的文本框）；「预览」只看渲染结果。切换模式不重建文本框，文字、光标和撤销记录都还在。每次打开回复框、每次点帖子上的「编辑」都从「编辑」模式开始：回复抽屉关上后内容仍留在页面上，所以每次打开给 `PostEditor` 换一个 `key`，草稿存在回复框里，换了也不丢；帖子的编辑框每次编辑都重新挂载。渲染结果就是 `ForumMarkdown`，和帖子在话题页上显示的一样，原始 HTML 同样当文字显示；所以别人写的文字（回复框预填的引用 `quoteDraft`、版主编辑别人的帖子）原样放进文本框，发出去的就是框里的文字。原来的 `TxMarkdownEditor` 会把内容用 DOMPurify 默认配置渲染在页面里（放行 `<style>`、`<form>`、`<input>`），为它加的 `toEditor`/`fromEditor`（#126、#134）随之去掉。论坛工具链没有 DOM 库，`tests/support/sfc.ts` 用 Vue 自己的编译器编译组件、挂到内存里的节点树上：`tests/post-editor.test.ts` 核对三个模式各显示什么、切换不丢字、分栏随输入更新、预览与 `ForumMarkdown` 的输出一致、原始 HTML 仍是文字；`tests/post-editor-flows.test.ts` 挂载回复框、`/new` 和帖子上的编辑框，核对引用或编辑含 `<style>`、`<form>` 的帖子在分栏和预览里不出标签、发出去的是框里的原文、字数上限照旧，从「预览」发出一条回复、或在「分栏」里取消之后再打开，回到「编辑」、文本框露出来、没发的草稿还在，以及编辑帖子时保存的是框里改过的文字、取消后再编辑又从「编辑」和帖子原文开始；`tests/editor-call-sites.test.ts` 读源码，核对三处都用 `PostEditor`、预览只经 `ForumMarkdown`，以及个人网站和资料保存经过共享检查。

接口路径、请求体、错误码与限流以核心服务为准，见 [API](../../architecture/API.md)「论坛」（随 #57 合入）；浏览器客户端在 `shared/forum-api.ts`，不带请求体的写接口不发 `Content-Type`（Fastify 会拒绝空 JSON）。单测：`tests/forum-api.test.ts`（客户端与返回值校验）、`tests/pow.test.ts`（与 `node:crypto` 同一结果、服务端能验证）、`tests/forum-server-store.test.ts`（假 fetch 下的加载、写入、失败回到服务端确认的样子、游客回复的请求体）、`tests/forum-optimistic.test.ts`（每一种写操作在请求返回前就显示、成功后保留、被拒绝时回滚并弹 toast；连点只有一个请求在路上；回复先以 `pending:` 编号显示）、`tests/forum-changes.test.ts`（`applyChanges` 原地合并，记录和列表对象不换）、`tests/access.test.ts`（游客/成员/离线各能做什么、版务只看 capabilities、头像菜单）、`tests/likes.test.ts`（「赞」的文字、赞数、按下状态，游客点了提示登录，服务端模式发一次请求并照返回的状态显示）、`tests/turnstile.test.ts`。
...
工具链选择见 [TUFF-FORUM](../../ops/TUFF-FORUM.md)；容器内由 `yzgc-<environment>/forum` 镜像提供静态产物，web 容器以 `proxy_pass http://forum:3000/`（尾斜杠剥离 `/forum` 前缀）反代，论坛发出的相对跳转由 `proxy_redirect / /forum/` 补回前缀。镜像的构建阶段 `node:26-bookworm-slim` 与运行阶段 `nginx:1.31-alpine` 在 `app/forum/Dockerfile` 里按 digest 固定（`<tag>@sha256:<digest>`，#97）；换基础镜像的步骤见 [DEPLOY](../../ops/DEPLOY.md)「基础镜像按 digest 固定」。
...
GEEK_FORUM_SOURCE=site GEEK_FORUM_BASE_PATH=/forum/ node scripts/forum.mjs generate  # 按镜像的方式构建极客班论坛，产物含每篇公开旧帖的 t/<id>.md 与 llms.txt
