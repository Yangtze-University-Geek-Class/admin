...
- **写正文**：新话题（`/new`）、回复框（含引用）和编辑帖子都用 `PostEditor.vue`（#144），上方三个模式，默认「编辑」：「编辑」是纯文本框，直接写 Markdown；「分栏」左边文本框、右边渲染结果，随输入更新，编辑器宽度不到 42rem（手机、窄屏、话题页里的编辑框）时改成上下排，两块平分原来的高度，所以回复抽屉里也放得下（会员回复抽屉高 460px，原来 420px 放不下模式切换加 240px 的文本框）；「预览」只看渲染结果。切换模式不重建文本框，文字、光标和撤销记录都还在。渲染结果就是 `ForumMarkdown`，和帖子在话题页上显示的一样，原始 HTML 同样当文字显示；所以别人写的文字（回复框预填的引用 `quoteDraft`、版主编辑别人的帖子）原样放进文本框，发出去的就是框里的文字。原来的 `TxMarkdownEditor` 会把内容用 DOMPurify 默认配置渲染在页面里（放行 `<style>`、`<form>`、`<input>`），为它加的 `toEditor`/`fromEditor`（#126、#134）随之去掉。论坛工具链没有 DOM 库，`tests/support/sfc.ts` 用 Vue 自己的编译器编译组件、挂到内存里的节点树上：`tests/post-editor.test.ts` 核对三个模式各显示什么、切换不丢字、分栏随输入更新、预览与 `ForumMarkdown` 的输出一致、原始 HTML 仍是文字；`tests/post-editor-flows.test.ts` 挂载回复框和 `/new`，核对引用或编辑含 `<style>`、`<form>` 的帖子在分栏和预览里不出标签、发出去的是框里的原文、字数上限照旧；`tests/editor-call-sites.test.ts` 读源码，核对三处都用 `PostEditor`、预览只经 `ForumMarkdown`，以及个人网站和资料保存经过共享检查。
