...
| `applications` | 投递简历 | portal `POST /api/portal/apply` | **没有任何读取接口**，维护者只能按数据运维流程直接读库 | `name`、`class_name`、`email`、`strengths`、`source_ip`、`user_agent`，全部属于候选人个人信息 |
| `audit_logs` | 审计记录 | `storage.audit()`，各路由调用 | admin `GET /api/admin/:org/logs`，只返回 `org = :org` 的行 | `ip` 列记录来源 IP。`org` 为空的记录（登录、登出、投递简历）不会通过任何接口返回 |
