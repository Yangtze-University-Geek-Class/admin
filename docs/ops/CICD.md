...
1. 同一 commit 在 `preview` 环境有成功的 GitHub deployment 记录（显式 deployment，不是分支或 tag 名称匹配）；
2. `https://prev.yangtzeu.work/release.json` 的 `environment` / `version` / `commit` 与本次发布逐字一致（`release.json` 由 web 镜像内置）——避免「上一个预发布通过」被推广到未经试用的新代码；
3. `production` GitHub Environment 已配置审批要求（REST 实测 protection rules，不信任 YAML 里写了 `environment: production` 这一行）；
4. `DEPLOY_TARGET_ENVIRONMENT` 等于 `production`（防环境级变量回落）。

构建后另外比对镜像内容与预发布验收过的产物一致（同一 commit 的镜像 digest）。上述都是机器一致性检查，**不构成、也不能代替** [RELEASES](../conventions/RELEASES.md) 要求的人工试用与明确批准。
...
- 任何工作流都不自动创建/移动/删除 tag，不自动升级 `package.json` 版本，不写放行状态。
...
- 不把 `stage` 的部署当作正式环境证据：正式部署必须能追到同一 commit 的预发布部署记录与人工试用结论。
