...
| 新论坛 | http://127.0.0.1:3456/ | 发现 `.tools/forum-runtime/<快照>/` 时显示极客班论坛内容（快照模式），否则为原仓浏览器 localStorage 演示；`GEEK_FORUM_SOURCE=demo` 强制示例（见 [TUFF-FORUM](TUFF-FORUM.md)）。Nuxt 开发服务器把 `/auth` 转给 3000（`app/forum/nuxt.config.ts` 的 `nitro.devProxy`），所以论坛能读 `/auth/me` |
