...
已生成独立只读投影并可在本机只读显示，但没有导入可写目标数据库、上线新论坛、建立真实统一登录、重新使用历史会话、运行持续同步，也没有发布（新发布模型不使用 tag，见 [RELEASES](../conventions/RELEASES.md)）。当前用户将步骤限定为“先拉到本地”，后续转换和上线是下一阶段，须另有副本演练、身份认领策略、权限/隐藏/删除数据处理、附件安全校验、增量核对及 [人工发版门禁](../conventions/RELEASES.md)。
