...
Branch model: only `main` (production) and `stage` (preview) are long-lived; task branches are created from `stage` and must be deleted right after merge. Confirm your branch before any write. See [BRANCHING.md](conventions/BRANCHING.md).
