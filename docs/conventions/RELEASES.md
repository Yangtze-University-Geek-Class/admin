...
- GitHub 计划让部署 job 失败关闭时，`scripts/deploy-manual.mjs` 部署同一 tag 的 CI 产物，同样只在上述授权之后运行；正式部署要把所有者的批准评论链接传给 `--acceptance`，评论的条件与格式只以 [CICD](../ops/CICD.md#维护者机器部署免费版的退路) 第 3 条为准。
