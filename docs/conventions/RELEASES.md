...
- GitHub 计划让部署 job 失败关闭时，`scripts/deploy-manual.mjs` 部署同一 tag 的 CI 产物，同样只在上述授权之后运行；正式部署要把所有者的批准评论链接传给 `--acceptance`：评论在本仓库的 issue 或 PR 里、作者是仓库管理员、发在预发布部署成功之后，并且有**单独一行**正好是 `批准发布 vX.Y.Z`（行尾可带句号；引用、否定句、只写 rc 的都不算）（见 [CICD](../ops/CICD.md#维护者机器部署免费版的退路)）。
