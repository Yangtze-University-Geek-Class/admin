...
- 每天巡检（`issue-lifecycle` 的 `sweep`，`scripts/issue-sweep.mjs`）：关联的 PR 已经合并进 `stage`、issue 还开着的，自动关闭并留「关闭」记录（合并后关过又被人重开的不再去关）；其余开着的 issue 14 天没有任何动静，留一条「超期」记录；14 天内关闭的 issue 既没有合并的 PR 也没有「关闭」记录的，留一条「缺记录」，请关闭的人补上，不重开。
