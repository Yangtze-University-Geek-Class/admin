...
scope 使用 `portal`、`forum`、`admin`、`shared`、`server`、`bot`、`auth`、`db`、`docs`、`deploy`、`tooling`、`deps`、`release`。按职责中心选择，不罗列全部文件：`portal`/`admin`/`shared` 属于 `app/web` 内部模块，`forum` 指 `app/forum`，`server` 指 `app/server`，`bot` 指组织审查机器人 `app/bot`，`docs`/`deploy`/`tooling` 分别指文档、部署模板与根脚本。
