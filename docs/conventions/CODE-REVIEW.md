...
组织审查机器人（[TRACKING](TRACKING.md) §6、[ADR-0004](../decisions/0004-review-bot-deploy-target.md)）会给每个 PR 发审查意见。它的意见只是参考：

- 只用 PR review 的「Comment」类型发，从不「Approve」或「Request changes」，也不合并。
- 条目格式与上文「产出格式」相同，严重度也用 `阻塞` / `应修` / `建议`；它写明被审查的提交和依据的规范。
- **它不写 `**结论：…**`，也不能充当本规范要求的审查结论**。没有人写的审查结论，PR 照样不能合并。人审查时可以采用机器人的条目，但要自己核对过 diff，并在结论里写明采用了哪几条。
