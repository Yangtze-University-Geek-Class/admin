...
5. **镜像与 compose 变更风险**：镜像 tag 语义（`<sha12>`）、端口是否与另一环境冲突（18100/18101 vs 18200/18201）、命名卷是否被改成宿主目录、健康检查是否仍在、`dockerfile:` 路径是否指向 `app/<service>/Dockerfile`、构建上下文是否仍为仓库根。
...
9. **旧模型残留**：diff 中不得把 `next` 分支、`feat/|fix/|docs/` 命名、`release-*`/`prev-*` tag、systemd、`/opt/yzgc-admin`、宿主 3000 端口当成现行模型（历史章节内须显式标注历史）。
