...
6. **测试与文档同步**：行为变更是否带来相应回归；改动的 API/环境变量/命令/路径是否同步到 [services/](../services/README.md)、[API](../architecture/API.md)、[ENVIRONMENTS](../ops/ENVIRONMENTS.md)、[README](../../README.md)、[TESTING](TESTING.md)；文档索引是否重新生成（`node scripts/docs-index.mjs --check`）。`pnpm check:doc-sync` 通过只说明对应的文档动过（[docs/README](../README.md)「文档跟着模块改」）：对照模块的改动核对文档改的内容。文档里的事实没变时，作者更新服务文档的「最近核对」一行，审查人对照 diff 确认它写的提交、改动和「其余说明不用改」的理由属实，属实就是合法的同步；只改「更新：」日期、既没改说明也没写这一行的是假同步，按 `阻塞` 处理。
