...
公开邀请/反馈/加入我们投递有限流、蜜罐、PoW 和可选 Turnstile。论坛的游客回复（`POST /api/forum/posts` 不带 `sid`）是另一个匿名写入口，同样要蜜罐、PoW（摘要输入 `${topicId}:${content}`）与可选 Turnstile，正文 ≤2000 字、昵称 1–20 字，昵称先做 NFKC、去掉首尾空白，再按允许清单收字符（`isAllowedName`），存的就是校验过的这个写法（不换行空格、全角空格这类字符存成普通空格，不会原样存下来显示得像没有空格）：汉字、常用拉丁字母（含拼音声调、越南文）、平假名、片假名、韩文（只收合成好的音节，单独的字母不收）、ASCII 数字，以及空格和 `- _ . · ・ '`，空格不能在首尾或连着用；看不见的字符、表情、其它文字（西里尔、希腊、亚美尼亚等的形近字母）和 IPA、小型大写这类形近拉丁字母都进不来。原来的屏蔽清单拦一种漏一种（第三轮审查又找到未分配的可忽略字符 U+2065、U+FFF0、U+E0080 和亚美尼亚、傈僳、切罗基的形近字母），所以换成允许清单。按 NFKC、不分大小写、去掉附加符号、把形近字折成同一个（ı 和 i、片假名カ和汉字力这类）、去掉所有空白和 `- _ . · ・ '` 之后（「极 客班」「极客班.」都算「极客班」），昵称不能与成员或官方账号的昵称、用户名相同，也不能是本地知道的组织成员的登录名：控制台里有称号的人（`role_assignments`）和登录过的人（审计里的 `auth.signin` 与当前会话，包括登录过的组织 owner），没打开过论坛的也算；成员改昵称同样只收这些字符，也不能等于官方账号的名字、别人的用户名或这些人里别人的登录名。昵称被占的回答（`guest_name_taken`）会透露某个名字是不是登录过或有称号的组织成员，所以这项检查放在限流和 PoW 之后；PoW 通过后每次尝试都记一次这个 IP 的回复额度，不管后面是昵称被占、人机验证没过还是发出去了，匿名探测和回复共用下面的额度、要先解 PoW，开了 Turnstile 时也分不出被占和没被占在额度上的差别。游客回复每个 IP 5 次/分钟、30 次/天，IPv6 按 /64 前缀计，全站游客回复合计每小时 200 次，超了所有游客暂停回复（`guest_replies_paused`），成员不受影响（都记在 `forum_rate_events`，重启不清零）；`GET /api/forum/state` 每个 IP 120 次/分钟、浏览接口 60 次/分钟（进程内计数）；游客不能发帖、编辑、删除或做其它写操作，每条游客回复是一个单独的游客用户，不能冒用成员身份。游客的来源 IP 只存在限流记录（最多一天）和浏览去重（最多一小时）里，不随帖子下发。反馈摘要与管理员回复经 `GET /api/feedback/public` 匿名可读：只要给出组织名即可读取该组织非 `spam` 反馈的前 280 字、状态、回复和票数，不校验 `ALLOWED_ORGS`；`limit` 由公共 querystring 校验限定为正整数、最多返回 100 条（见 [API](API.md) 端点清单）；反馈正文因此按公开内容对待，用户指南已提示勿提交敏感信息。「加入我们」投递没有任何读取接口。邀请先预留额度、再请求上游，明确失败补偿、未知结果保留待核对，不把超时解释为未发送。写请求核对 Origin 与 Fetch Metadata，返回错误不打印 token 或完整上游响应。鉴权 API 禁止缓存。公开文档使用允许列表，不公开内部运维、安全、规范和审查内容。论坛昵称的冒名检查还剩一个缺口：从没登录过、也没有称号的组织成员（包括从没登录过的组织 owner），本地没有这些人的登录名，游客和成员仍能用这些名字。
