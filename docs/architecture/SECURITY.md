...
- **论坛能力在服务端执行**（#57）：`/api/forum/*` 用同一个 `computeAccess` 算出的 `forum.*` 判定置顶（`forum.topic.pin`）、关闭（`forum.topic.close`）、编辑或删除他人帖子与在已关闭话题回复（`forum.post.moderate`），控制台里改过的称号与部门权限包下一次请求就生效；`forum.category.manage` 与 `forum.badge.assign` 仍没有接口。`state.viewer.capabilities` 下发给前端只用于显示按钮，授权只在服务端。动他人帖子、置顶、关闭以 `org = CONSOLE_ORG` 写审计，details 只有帖子与话题编号、作者编号或开关，不记正文。
