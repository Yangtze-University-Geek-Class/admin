...
- **写接口的回答**（#145）：不再是整份 `state`，是 `{ changes, viewer, guestPolicy }`（发话题另带 `topicId`、`postId`，回复另带 `postId`）。`viewer`、`guestPolicy` 与 `state` 里的相同；`changes` 只有这次写入改动的记录，按类放在 `users`、`tags`、`topics`、`posts`、`notifications`、`bookmarks`、`follows` 里（没有的类不出现），每条与同一个人此刻读 `state` 得到的那条完全一样，可见性规则也一样：别人的 `notifyPrefs` 是初始值，别人的通知和书签不会出现。取消的书签和关注只有两端的编号，放在 `removed.bookmarks: [{ userId, postId }]`、`removed.follows: [{ followerId, followeeId }]`。成员的回答总带自己的用户记录（每次请求都会刷新角色、称号和 GitHub 头像）。每个接口带哪些记录写在下表「成功」一栏，也写在 `routes/forum-api/contracts.ts` 的 `ForumWriteResponse`。前端按编号把 `changes` 并进手里的状态，见 [forum 合同](../services/forum/README.md)「服务端模式」。
...
- **昵称**：游客昵称与成员的 `displayName` 用允许清单（`lib/forum-rules.ts` 的 `isAllowedName`，先做 NFKC，全角字母和数字变成半角；存的是 NFKC 之后去掉首尾空白的写法，不换行空格、全角空格存成普通空格）：只收汉字、平假名、片假名、韩文（只收合成好的音节 U+AC00–U+D7A3；单独的字母和显示成空白的填充字不收，按顺序写的字母 NFKC 会合成音节）、拉丁字母里的基本拉丁、拉丁-1、拉丁扩展 A、拼音声调字母 U+01CD–U+01DC 与 U+1E00–U+1EFF、ASCII 数字、长音符 ー，以及空格和 `- _ . · ・ '`；空格不能在首尾或连着用，至少要有一个字或数字。其它都拒绝：看不见的字符、表情、西里尔与希腊等其它文字、IPA 与小型大写这类形近拉丁字母；NFKC 之后 `µ` 是希腊字母，也拒绝。错误码是 `invalid_guest_name` / `invalid_display_name`，`message` 是「昵称只能用汉字、字母、假名、韩文、数字、空格和 - _ . · ・ ' 这几个符号，空格不能连着用」，空名字另有长度提示。规则只管新写入，库里已有的昵称不改；成员提交的 `displayName` NFKC 并去掉首尾空白后，和库里现存的（同样处理）相同，就当没改，不查也不写，不合现在规则的旧昵称不会挡住改签名、网站。判断重名时两边都按 `nameKey` 归一：NFKC、不分大小写、去掉附加符号（é 和 e、が 和 か 算同一个），形近字折成同一个（ı 和 i、ĸ 和 k，片假名カ ニ ロ エ ハ タ ト ー 和汉字力 二 口 工 八 夕 卜 一，平假名へ和片假名ヘ），再去掉所有空白和 `- _ . · ・ '`（「极 客班」「极客班.」和「极客班」、「阿・凡提」和「阿·凡提」算同一个）。
...
| `POST /api/forum/topics` | 成员 | 每人 10 次/分钟 | 201 `{ changes, viewer, guestPolicy, topicId, postId }`，`changes` 有新话题、首帖和话题用到的标签；body `{ title(1–120，去首尾空白后非空), categoryId, tags?: string[](≤5，每项是已有标签 id 或 ≤20 字的名字), content(1–20000) }`。名字按 slug（英文）或名字（中文，不分大小写）找已有标签，找不到才新建 | 401 `signin_required`；400 `validation_error` / `invalid_title` / `unknown_category` / `invalid_tag` / `empty_content`；429 `rate_limited` |
| `POST /api/forum/posts` | 成员、游客 | 成员每人 30 次/分钟；游客每个 IP 5 次/分钟、30 次/天（按通过 PoW 的尝试计，昵称被占、人机验证没过也算），全站游客合计 200 次/小时（只算发出去的） | 201 `{ changes, viewer, guestPolicy, postId }`，`changes` 有新帖子和它所在的话题（最后活动时间变了），游客回复另有这次新建的游客用户；body `{ topicId, content, replyToPostId? }`，游客另带 `guest: { name(1–20) }`、`pow: { timestamp, nonce }`、蜜罐 `website`（必须为空）、`turnstileToken`（`guestPolicy.turnstileSiteKey` 非空时必填）。PoW 摘要输入是 `${topicId}:${content}`（正文原样，不去空白），难度同 `pow_difficulty`；游客正文 ≤2000、成员 ≤20000。话题已关闭时只有持 `forum.post.moderate` 的人能回 | 404 `not_found`；403 `forbidden`（话题已关闭）；400 `validation_error` / `invalid_reply_target`（被回复的帖子不在这个话题）/ `empty_content` / `request_rejected`（蜜罐）/ `content_too_long` / `invalid_guest_name`（空、超长或有允许清单以外的字符）/ `guest_name_taken`（按 `nameKey` 与成员或官方账号的昵称、用户名相同，或是本地知道的组织成员的登录名：控制台里有称号的人、登录过的人，没打开过论坛的也算；这项在限流和 PoW 之后才查，这时这次尝试已经记进这个 IP 的额度）/ `pow_invalid` / `turnstile_failed`；429 `rate_limited`（这个 IP 超了）/ `guest_replies_paused`（全站游客回复超了，所有游客暂停，成员照常） |
| `PATCH /api/forum/posts/:id` | 作者（成员）或 `forum.post.moderate` | 无 | 200，`changes` 有这条帖子；body `{ content }`，写 `editedAt`；改他人帖子审计 `forum.post.edit`（details `{ topic_id, author_id }`） | 401；403 `forbidden`；404；409 `post_deleted`；400 `empty_content` |
| `DELETE /api/forum/posts/:id` | 作者（成员）或 `forum.post.moderate` | 无 | 200，`changes` 有这条帖子，软删除；已删的再删直接返回它现在的样子；删他人帖子审计 `forum.post.delete` | 401；403；404；400 `first_post`（话题的第一帖不能删） |
| `POST /api/forum/posts/:id/like` | 成员 | 无 | 200，`changes` 有这条帖子，切换 | 401；404；409 `post_deleted` |
| `POST /api/forum/posts/:id/bookmark` | 成员 | 无 | 200，切换；加上时 `changes.bookmarks` 有这条书签，取消时在 `changes.removed.bookmarks` | 401；404 |
| `POST /api/forum/users/:id/follow` | 成员 | 无 | 200，切换；关注时 `changes.follows` 有这条关注，取消时在 `changes.removed.follows` | 401；404；400 `cannot_follow_self` |
| `POST /api/forum/topics/:id/pin` | `forum.topic.pin` | 无 | 200，`changes` 有这个话题；body `{ pinned: boolean }`；审计 `forum.topic.pin`（details `{ pinned }`） | 401；403 `forbidden`；404 |
| `POST /api/forum/topics/:id/close` | `forum.topic.close` | 无 | 200，`changes` 有这个话题；body `{ closed: boolean }`；审计 `forum.topic.close`（details `{ closed }`） | 401；403；404 |
| `POST /api/forum/topics/:id/view` | 所有人 | 每个 IP 60 次/分钟（进程内计数）；同一 IP 同一话题 1 小时只算一次浏览 | 204 | 404；429 `rate_limited` |
| `POST /api/forum/notifications/:id/read` | 成员 | 无 | 200，`changes` 有这条通知 | 401；404（不存在或不是自己的） |
| `POST /api/forum/notifications/read-all` | 成员 | 无 | 200，`changes` 有这次从未读变成已读的通知（本来都读过时没有 `notifications` 这一类） | 401 |
| `PATCH /api/forum/me/profile` | 成员 | 无 | 200，`changes` 只有自己的用户记录；body 至少一项：`displayName`（NFKC 并去首尾空白后 1–30 个字，只收允许清单里的字符；按 `nameKey` 不能等于官方账号的昵称或用户名、别人的用户名，或控制台里有称号、登录过的别人的登录名，和别的成员昵称相同可以；自己的登录名本身（不分大小写）总能用，但只豁免它本身，`da-ve` 不能用 `dave`）、`bio`（个人签名 ≤200，可换行）、`location`（≤60）、`website`（空串清空，否则 `https://`、≤200、不带账号密码）、`notifyPrefs: { reply?, like?, follow? }` | 401；400 `validation_error` / `invalid_display_name` / `display_name_taken` / `invalid_bio` / `invalid_location` / `invalid_website` |
| `PUT /api/forum/me/avatar` | 成员 | 每人 10 次/小时，每次上传都计数（解码失败也算） | 200，`changes` 只有自己的用户记录；登录与次数在读请求体之前（onRequest）核对，游客和超了次数的请求体不会被读进内存。请求体是图片本身，`Content-Type` 为 `image/png`、`image/jpeg` 或 `image/webp`，≤2MB。sharp 解码（像素上限 4096×4096，超了在解码前拒绝；动图只取第一帧），按 EXIF 摆正，居中裁成正方形，缩到 256×256 WebP，按内容 sha256 存库；换头像后没人用的旧图删除 | 401；415 `unsupported_media_type`；413 `avatar_too_large`；400 `invalid_image` / `image_too_large`；429 |
| `DELETE /api/forum/me/avatar` | 成员 | 无 | 200，`changes` 只有自己的用户记录，回到 GitHub 头像 | 401 |
...
表里写 200 没写形状的写接口，回答都是上面「写接口的回答」的 `{ changes, viewer, guestPolicy }`。
