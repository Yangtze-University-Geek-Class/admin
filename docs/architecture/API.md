...
| `PATCH /api/console/titles/:title_id` | `roles.manage` | 无 | 200 `{ title: { id, label, tag, icon, tone, description, capabilities } }`，审计 `title.update`（details `{ changed }`）；body 是 `label`（1–8 字）、`tag`（`^[A-Z][A-Z0-9-]{1,15}$`）、`icon`（`department_icons` 之一）、`tone`、`description`（≤200 字）、`capabilities` 的非空子集 | 400 `validation_error`（未知称号、未知字段、空 body）/ `title_capabilities_fixed`（改 admin 或 guest 的权限）/ `captain_only_capability`（`roles.manage` 放进 captain 以外的包）；403 `missing_capability` |
